...
def crawl(
    ...
    try:
        ...
        while page < max_pages:
            ...
            if "just a moment" in html[:2000].lower() or "cf-chl" in html[:5000].lower():
                print("  Got a Cloudflare challenge page instead of real content — "
                      "the fallback browser didn't clear it in time. Try re-running; "
                      "if it keeps happening, a visible Chrome window should have "
                      "popped up — check whether it needs a manual click.")
                break
